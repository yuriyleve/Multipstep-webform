...
<template>
    <div>
        <form @submit.prevent="handleSubmit">
            <div class="mb-6">
                <label for="container_domain"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Domain</label>
                <input name="container_domain" id="container_domain" :value="store.container.domain"
                    @input="event => store.update({ domain: event.target.value })"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="" required>
            </div>
            <div class="mb-6">
                <label for="container_subdomain"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Subdomain</label>
                <input name="container_subdomain" id="container_subdomain" :value="store.container.subdomain"
                    @input="event => store.update({ subdomain: event.target.value })"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder="" required>
            </div>
            <router-link to="/ct/info/plan">
