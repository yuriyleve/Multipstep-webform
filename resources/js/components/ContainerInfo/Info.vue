        ...
        <form @submit.prevent="handleSubmit">
            <div class="mb-6">
                <label for="container_name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Container
                    name</label>
                <input name="container_name" id="container_name" :value="container.name"
                    @input="event => container.update({ name: event.target.value })"
            ...
            <div class="mb-6">
                <label for="container_config" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Container
                    config</label>
                <input name="container_config" id="container_config" :value="container.config"
                    @input="event => container.update({ config: event.target.value })"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
            <div class="mb-6">
                <label for="container_location"
                    class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Container location</label>
                <select name="container_location" id="container_location" :value="container.location"
                    @input="event => container.update({ location: event.target.value })"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
